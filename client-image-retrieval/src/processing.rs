...
pub fn get_f16_to_f32_lut(val: u16) -> f32 {
    F16_TO_F32_LUT.get_or_init(create_f16_to_f32_lut)[val as usize]
}
...
fn get_f32_to_f16_lut(val: f32) -> u16 {
    ...
    F32_TO_F16_LUT.get_or_init(create_f32_to_f16_lut)[index]
}
...
pub fn get_f16_lut() -> &'static [u16; 256] {
    F16_LUT.get_or_init(create_f16_lut)
}
...
pub fn get_f32_lut() -> &'static [f32; 256] {
    F32_LUT.get_or_init(create_f32_lut)
}
            ...
            for y in 0..letterbox.new_height {
                ...
                while x + 4 <= width {
                    unsafe {
                        ...
                        // Pixel 0
                        *out_r.get_unchecked_mut(dst_idx0) =
                            *r_lut_ptr.add(*in_ptr.add(src_idx0) as usize);
                        *out_g.get_unchecked_mut(dst_idx0) =
                            *g_lut_ptr.add(*in_ptr.add(src_idx0 + 1) as usize);
                        *out_b.get_unchecked_mut(dst_idx0) =
                            *b_lut_ptr.add(*in_ptr.add(src_idx0 + 2) as usize);

                        // Pixel 1
                        *out_r.get_unchecked_mut(dst_idx1) =
                            *r_lut_ptr.add(*in_ptr.add(src_idx1) as usize);
                        *out_g.get_unchecked_mut(dst_idx1) =
                            *g_lut_ptr.add(*in_ptr.add(src_idx1 + 1) as usize);
                        *out_b.get_unchecked_mut(dst_idx1) =
                            *b_lut_ptr.add(*in_ptr.add(src_idx1 + 2) as usize);

                        // Pixel 2
                        *out_r.get_unchecked_mut(dst_idx2) =
                            *r_lut_ptr.add(*in_ptr.add(src_idx2) as usize);
                        *out_g.get_unchecked_mut(dst_idx2) =
                            *g_lut_ptr.add(*in_ptr.add(src_idx2 + 1) as usize);
                        *out_b.get_unchecked_mut(dst_idx2) =
                            *b_lut_ptr.add(*in_ptr.add(src_idx2 + 2) as usize);

                        // Pixel 3
                        *out_r.get_unchecked_mut(dst_idx3) =
                            *r_lut_ptr.add(*in_ptr.add(src_idx3) as usize);
                        *out_g.get_unchecked_mut(dst_idx3) =
                            *g_lut_ptr.add(*in_ptr.add(src_idx3 + 1) as usize);
                        *out_b.get_unchecked_mut(dst_idx3) =
                            *b_lut_ptr.add(*in_ptr.add(src_idx3 + 2) as usize);
                    }
                    x += 4;
                }
                ...
                while x < width {
                    unsafe {
                        ...
                        *out_r.get_unchecked_mut(dst_idx) =
                            *r_lut_ptr.add(*in_ptr.add(src_idx) as usize);
                        *out_g.get_unchecked_mut(dst_idx) =
                            *g_lut_ptr.add(*in_ptr.add(src_idx + 1) as usize);
                        *out_b.get_unchecked_mut(dst_idx) =
                            *b_lut_ptr.add(*in_ptr.add(src_idx + 2) as usize);
                    }
                    x += 1;
                }
            }
