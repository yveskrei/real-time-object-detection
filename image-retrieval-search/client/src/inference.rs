...
// Custom modules
use crate::utils::config::{AppConfig, DeviceType, ModelConfig, ModelPurpose, TritonConfig, InferencePrecision};
    ...
    pub fn model(&self, model_purpose: ModelPurpose) -> Result<Arc<InferenceModel>> {
        Ok(
            self.models
                .get(&model_purpose)
                .cloned()
                .context("Inference model is not initiated!")?
        )
    }
...
impl InferenceModel {
    ...
    /// hardware we run on and the purpose the model serves - so a deployment can tune
    /// the count per GPU model, or for CPU, and per model purpose.
